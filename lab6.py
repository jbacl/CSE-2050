class OrderedList:
    ...
    def _bs(self, item, left=None, right=None):
        ...
        if self._L[left] == item or self._L[right-1] == item: return True
        # item is in smaller half
        elif item == self._L[(right-1)//2]:
            return True
        elif item < self._L[(right-1)//2]:
            return self._bs(item, 0, (right-1)//2)
        # item is in bigger half
        elif item > self._L[(right-1)//2]:
            return self._bs(item,(right-1)//2, right-1)